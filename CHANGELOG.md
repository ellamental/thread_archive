...
- Security pass over untrusted inputs: a dropped ZIP is read against a decompression ceiling, `archive-mcp --http` owns
  its DNS-rebinding allow-list, `git` is pinned off repository config, and a blob serves the type its URL named.
- Additive drift waits two weeks before it warns, and `source recheck` retires a verdict a re-read repaired; `source fix
  --activate`'s re-import stops being a no-op that reported success.
