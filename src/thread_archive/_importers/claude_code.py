...
def _is_archive_operational(all_lines: list[dict]) -> bool:
    ...
    the archive's own machinery spawned (scheduled background jobs run from
    inside ``<home>``), not operator work. Filed as hidden
    ``thread_type='system'`` threads with ``exclude_from_search`` set: captured,
    but never surfaced by search (such a job's transcript is full of quoted search
    hits, so it would match nearly any query about its own subjects) and never
    themselves treated as new work — otherwise every job's own transcript re-enters
    the work queue and future jobs summarize past ones, forever."""
    ...
    *,
    ...
    if thread_id is None:
        ...
        thread_id = create_thread(
            ...
            # Operational (background-job) transcripts quote search hits
