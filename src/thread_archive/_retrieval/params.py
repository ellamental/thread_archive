...
- ``fusion_weight`` 400.0 — the cross-backend fusion term (the normalized
  ...
  no purchase, are the ones that move. **This is the biggest ranking term measured
  on this bench**: against ``fusion_weight=0`` it is +.023 MRR / +.011 recall@10 /
  +.024 nDCG@10 pooled over all 25 gold files, more than any other knob moves, and
  it buys ordering as much as reach (findability MRR .639 → .726). **400 is the
  pooled argmax, not a per-file optimum** — swept over {0, 300, 400, 500} it takes
  pooled nDCG@10 (.602 / .623 / .627 / .624) and recall@10 and ties the best
  success@10, but only 10 of the 25 files peak there, 8 peaking at 300 and 7 at
  500. Past ~500 the vector arm starts overriding lexical
...
- ``bm25_weight`` 100.0 — the weight on ``_lex``, the lexical arm's own placement
  ...
  **The two corpora disagree about it, and the hold-out is why it stays.**
  Scored against ``bm25_weight=0`` at the rest of the shipped configuration, the
  archive's own golds no longer separate the term from 0: +.030 MRR / +.021
  nDCG@10 on the recall-capable ``judged`` file and +.008 MRR on findability,
  against −.025 MRR / −.010 nDCG@10 on ``rerank`` and a little topic recall,
  netting −.002 recall@10 and ±.000 nDCG@10 pooled over all 25 files. The gains
  it was originally set for predate ``bm25_score_weight`` and ``semantic_weight``,
  which now carry much of the discrimination it was the counterweight for. The
  SWE-chat hold-out still credits it: +.011 MRR / +.007 nDCG@10 / +.003 recall@10
  pooled, up on the provenance-labeled ``commit`` file and the ``topic`` files
  alike, against −.013 success@10 on its findability file. Nothing is tuned
  against that corpus, so its verdict is the one that carries — and the direction
  is bounded either way: past ~400 bm25's order starts overriding the density
...
- ``bm25_score_weight`` 100.0 — the weight on ``_bm25``, FTS5's own bm25 score for
  ...
  term does not have. Against 0 at the rest of the shipped configuration it now
  reads as an ordering term that costs a little reach: ``judged`` +.021 nDCG@10 /
  +.030 MRR and findability +.007 nDCG@10, against ``rerank`` −.006 nDCG@10 /
  −.007 recall@10, netting +.001 nDCG@10 and −.002 recall@10 pooled over all 25
  files. It is worth more paired with ``semantic_weight`` than the two are apart —
  together they are +.020 recall@10 / +.031 nDCG@10 on the three protocol files,
  more than either buys alone. Past ~200 the confound-dense topic files give back
  recall as bm25's verdict starts overriding the density evidence they lean on.
- ``semantic_weight`` 200.0 — the weight on the vector arm's cosine, spread
  ...
  moves recall as well as order: against 0 it is +.009 recall@10 / +.018 nDCG@10 on
  the three protocol files and +.006 / +.011 pooled over all 25, concentrated in
  findability (+.033 MRR / +.029 nDCG@10) — the vocab-mismatch shapes the lexical
  arm has no purchase on. It is not free: ``judged`` gives back .011 MRR for it.
  Past ~400 it starts overriding lexical evidence it should defer to and the
  recall-capable ``judged`` file gives back a case; the raw cosine is worth
...
- ``thread_evidence_weight`` 0.0 — off. The signal (how many distinct matches the
  ...
  queries: +.022 nDCG@10 / +.017 recall@10 over the 22 ``topic`` gold files — the
  multi-answer protocol, where a completeness gain has room to show. It ships off
  because of what it trades for
  ...
  single-mention answer. A query-shape gate is the seam that would earn it; until
  then the recall-capable file's verdict stands.
