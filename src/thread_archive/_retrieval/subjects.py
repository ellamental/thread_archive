...
It annotates, never reorders — so it cannot regress ranking, and a wrong label
is cheap and visible where a silently-buried good result would be expensive
and invisible. Coverage across the result set ranks the subjects; specificity
(IDF over how many chats a subject links corpus-wide) damps the broad,
stopword-like subjects that touch everything. Fail-soft and a strict no-op
