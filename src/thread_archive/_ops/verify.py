...
def verify(
    *,
    ...
    drift (truth > index) is the *safe* direction — ``thread_archive reindex`` rebuilds
    ...
    is cleared by ``thread_archive repair``, which quarantines the damaged lines and
    ...
    **cross-store payload parity** check: for every event id present on both
    ...
    The index is rebuildable, so a failure here means ``thread_archive reindex``, not
    ...
    # Record the outcome in the home's health file so `thread_archive status` (and
    ...
    # the mirror was parse-scanned — the check a bare `thread_archive verify` skips.
...
def _verify_deep(watermark: int) -> dict:
    ...
    unfindable (``unindexed_events``) and fails — ``thread_archive reindex`` rebuilds the
    surface.
    ...
    with get_session() as s:
        ...
        # so the event is silently unfindable — real drift; `thread_archive reindex`
