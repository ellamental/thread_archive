...
_MODULE_TEMPLATE = '''"""Override patch for the {name} provider — scaffolded by `thread-archive source fix`.
...
Pre-wired by `thread-archive source fix`; activation re-runs it and refuses a red
...
def _write_protocol(provider_name: str, target_dir: Path) -> None:
    ...
    (target_dir / "PROTOCOL.md").write_text(
        ...
        f"`thread-archive source fix {provider_name} --activate` succeeding\n",
        encoding="utf-8",
    )
...
def scaffold(provider_name: str, home: Optional[str] = None) -> Path:
    ...
    if provider is None:
        raise ValueError(
            f"unknown provider {provider_name!r} — `thread-archive source list` lists them"
        )
