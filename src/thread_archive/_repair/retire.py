...
An ``thread_archive fix-import`` patch exists to bridge the gap until a core release
...
``thread_archive fix-import``; the loop self-corrects with no intelligence required.
Pinning (``thread_archive fix-import <provider> --pin``) is the explicit opt-out for
    ...
    if retired:
        ...
        for name in retired:
            ...
            logger.info(
                ...
                "`thread_archive fix-import %s` rebuilds it against the new core.",
