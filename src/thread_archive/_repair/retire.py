...
An ``archive fix-import`` patch exists to bridge the gap until a core release
...
``archive fix-import``; the loop self-corrects with no intelligence required.
Pinning (``archive fix-import <provider> --pin``) is the explicit opt-out for
    ...
    if retired:
        ...
        for name in retired:
            ...
            logger.info(
                ...
                "`archive fix-import %s` rebuilds it against the new core.",
