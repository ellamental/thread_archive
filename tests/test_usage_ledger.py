...
def test_contention_sample_reports_only_what_it_has_to_say(archive_home) -> None:
    ...
    # none of the conditional fields appear. What remains is the four with no
    ...
    # and what this process is costing it — the last as a pair, since peak alone
    # never falls and so can never report memory the process has lost.
    assert set(_contention.sample()) == {"uptime_s", "load1", "rss_mb", "rss_now_mb"}
