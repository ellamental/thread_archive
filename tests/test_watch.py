...
def test_watch_once_holds_shared_ingest_lock(archive_home, tmp_path, monkeypatch, capsys):
    """``thread-archive watch --once`` holds the shared ingest lock like every other truth
...
def test_poll_errors_surface_in_health(archive_home) -> None:
    """A failing source must be visible to `thread-archive status` (health.json), not
