...
def test_each_fault_names_its_remedy():
    ...
    assert by_key["nightly-failed"]["command"] == "thread_archive nightly '/Volumes/back up/arc'"
    assert by_key["same-disk"]["tone"] == "warn"
    assert by_key["same-disk"]["command"].startswith("thread_archive daemon install")
...
def test_a_provider_failing_inside_a_fresh_pass_is_not_hidden_by_it():
    ...
    assert notices[0]["command"] == "thread_archive fix-import codex"
