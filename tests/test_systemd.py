...
def test_watcher_unit_shape() -> None:
    ...
    assert "ExecStart=/opt/venv/bin/archive watch --web --web-port 8787" in text
...
def test_backup_units_shape() -> None:
    ...
    assert "ExecStart=/opt/venv/bin/archive nightly /vol/bak" in svc
...
def test_backup_units_options() -> None:
    ...
    svc = units["thread-archive-backup.service"]
    assert "ExecStart=/opt/venv/bin/archive nightly /vol/bak --notify-url http://127.0.0.1:8002/api/notify" in svc
