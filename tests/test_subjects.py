"""The relevant-subjects lens — the topic graph as orientation over a result set.
...
def _link(topic_id: int, ev_tid: tuple[int, int]) -> None:
    knowledge.add_topic_evidence(topic_id, ev_tid[0], ev_tid[1], "cite")
...
def _seed(archive_home):
    ...
    broad = knowledge.create_topic("Broad Subject")["topic_id"]
    solo = knowledge.create_topic("Solo Subject")["topic_id"]
    for ev in (a, b):
