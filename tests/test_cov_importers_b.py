"""Branch-coverage tests for the bulk/DB importers + the topic knowledge graph.

Cluster: ``_importers/exports.py`` (claude.ai/ChatGPT/xAI account exports),
``_importers/opencode.py``, ``_importers/claude_science.py``,
``_importers/antigravity.py``, ``_importers/_titles.py`` and
``_knowledge/graph.py``.
...
def test_opencode_from_payload_empty_messages_noop(archive_home) -> None:
    ...
    assert res.events_created == 0 and res.thread_id == 0
...
# ══════════════════════════════════════════════════════════════════════════════
# graph.py — topic knowledge graph projections
# ══════════════════════════════════════════════════════════════════════════════
...
def _seed_graph() -> None:
    ...
    with get_session() as s:
        ...
        s.add(Thread(id=99, name="conv-99", title="A Conversation", thread_type="conversation"))
        s.flush()
        edges = [
            ...
            (1, 99, "related"),         # target not a topic → skipped
        ]
        for src, tgt, lt in edges:
            s.add(ThreadLink(source_thread_id=src, target_thread_id=tgt, link_type=lt))
        s.commit()
    kg.reset_cache()
...
def test_graph_status_and_metadata(archive_home) -> None:
    ...
    assert status["nodes"] == 7  # the non-topic thread is not a node
    assert status["communities"] >= 2
    # Batch metadata: a real topic resolves, an unknown id is skipped.
    meta = kg.get_topic_graph_metadata([1, 999])
    assert 1 in meta and 999 not in meta
    assert meta[1]["degree"] >= 2 and meta[1]["link_count"] == meta[1]["degree"]
    assert kg.get_topic_graph_meta(1)["pagerank"] > 0


def test_graph_community_queries(archive_home) -> None:
    _seed_graph()
    # A topic's community members (self excluded).
    cid = kg._projection().community.get(1)
    members = kg.get_community_topic_ids(cid)
    assert 1 in members
    peers = kg.get_community_peers(1)
    assert peers and all(p["thread_id"] != 1 for p in peers)
    # community_members_for: known topic yields members, isolated topic yields none,
    # unknown id is skipped entirely.
    mapping = kg.community_members_for([1, 7, 1000])
    assert 1 in mapping and 7 not in mapping and 1000 not in mapping


def test_graph_bridges_and_unconnected(archive_home) -> None:
    _seed_graph()
    bridge_ids = {b["topic_id"] for b in kg.get_bridge_topics()}
    assert {3, 4} <= bridge_ids  # the bridge endpoints carry the betweenness
    unconnected = kg.get_unconnected_topics()
    assert {u["thread_id"] for u in unconnected} == {7}
...
def test_graph_functions_return_empty_when_unavailable(archive_home, monkeypatch) -> None:
    ...
    assert kg.get_community_peers(1) == []
    assert kg.community_members_for([1]) == {}
...
def test_graph_community_peers_unknown_thread(archive_home) -> None:
    ...
    assert kg.get_community_peers(123456) == []
