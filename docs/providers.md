...
$ /path/to/archive/.venv/bin/thread-archive providers
...
`kind` says how `thread-archive import --provider <name>` reaches your importer.
...
distinct from anything already in `thread-archive providers`.
...
`thread-archive fix-import <provider>` sets the repair case up and gates it: it
