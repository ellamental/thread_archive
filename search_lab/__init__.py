...
Not part of the installed product. Nothing under ``src/thread_archive`` imports
this directory — the dependency runs one way, lab → package, so an install has no
path to the bench and the bench is free to reach into package privates.
...
* **a module** — ``import search_lab.eval_core``, which is how the tests, the
  gold gate under ``scripts/``, and the ``_mine`` miners reach the shared cores
