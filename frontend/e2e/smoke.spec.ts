...
test('the rail links out to the dev panels when the operator asked for it', async ({ page }) => {
  ...
  await page.goto('/')
  const link = page.getByRole('link', { name: /dev panels/ })
  // An absolute href to the panels' own server: the link leaves this origin
  // rather than asking this app for a route it does not have.
  await expect(link).toHaveAttribute('href', 'http://127.0.0.1:8789')
