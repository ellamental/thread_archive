---
name: gardener
description: "Tend the archive's topic graph: merge near-duplicate topics, connect or retire singleton islands, grow the part-of hierarchy, archive uncited husks. The structural counterpart to /librarian (which curates conversations). Triggers on: run the gardener, tend the graph, merge duplicate topics, clean up topics, organize the topic hierarchy."
...
If the invocation carries a number (`/gardener 30`), treat it as this run's
cap on write actions — merge / link / archive / rename each count as one —
then stop and report. No number: drain until the queues are clean.
...
- No conversation curation: no citations, no summaries, no thread edits —
  that's the librarian's job (`/librarian`).
